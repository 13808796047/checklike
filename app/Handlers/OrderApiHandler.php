...
//生成api请求的header信息
    ...
    public function createOrder($order, $file)
    {
        ...
        switch ($order->category->classid) {
            case 4:
                $data['author'] = mb_substr($order->writer, 0, 5, 'utf-8');
                break;
            default:
                $data['author'] = $order->writer;
        }
...
//        return file_put_contents(public_path().'/test.docx',$response->getbody()->getContents());
    ...
    public function cloudConvertFile($order)
    {
...
//            stream_copy_to_stream($source, $dest);
            $result = app(FileUploadHandler::class)->saveTxt($source, 'files', $order->user->id);
            return $result['path'];
        }
    }
}
